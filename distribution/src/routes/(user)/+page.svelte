<script lang="ts">
	...
	type Mode = 'all' | 'all_unpaid' | 'csv' | 'manual';
	...
	const attachSse = (id: string) => {
		...
		state.eventSource.addEventListener('log', (event) => {
			...
			const exists = state.logs.some(
				(item) => item.ts === entry.ts && item.message === entry.message
			);
			if (!exists) {
		...
		state.eventSource.addEventListener('issue', (event) => {
			...
			const exists = state.errors.some(
				(item) =>
					item.ts === entry.ts && item.contactId === entry.contactId && item.reason === entry.reason
			);
			if (!exists) {
	...
	const editorAction: Action<HTMLDivElement, EditorParams> = (node, params) => {
		const editor = new Editor({
			element: node,
			extensions: [StarterKit, Underline, Link.configure({ openOnClick: false }), Spoiler],
...
{#if state.step === 1}
	<section class="grid gap-6 lg:grid-cols-3">
		<div class="card space-y-4 lg:col-span-2">
			...
			<div class="grid gap-4 md:grid-cols-2">
				...
				<label class="flex items-center gap-3">
					<input
						...
						name="mode"
						value="all_unpaid"
						checked={state.mode === 'all_unpaid'}
						onchange={onModeChange}
						disabled={state.isLocked}
					/>
					<span class="body-m">Всем кроме оплативших</span>
				</label>
				<label class="flex items-center gap-3">
					<input
						type="radio"
			...
			{#if state.mode === 'manual'}
				<div>
					<label class="body-s text-text-muted" for="manual"
						>Список ID (каждый в новой строке)</label
					>
					<textarea
			...
			{#if state.mode === 'csv'}
				<div>
					<label class="body-s text-text-muted" for="csv"
						>Текстовый файл с ID (каждый на новой строке)</label
					>
					<input
						class="input mt-2"
						id="csv"
			...
			{/if}
			{#if state.mode === 'all_unpaid'}
				<p class="body-s text-text-muted">
					Отправка всем пользователям, которые не оплатили и не заблокировали бота.
				</p>
			{/if}
		</div>
		...
		<div class="card space-y-8">
						...
						{#each state.mediaItems as item}
							<div class="rounded-md border border-border bg-surface-2 p-3">
								{#if item.type === 'photo'}
									...
									{:else}
										<div
											class="flex h-32 w-full items-center justify-center rounded-md border border-border bg-surface text-text-muted"
										>
			...
			<div class="space-y-3">
				...
				<div class="rounded-md border border-border bg-surface p-3">
					<div
						class="editor-toolbar flex flex-wrap items-center gap-1 rounded-md border border-border bg-surface-2 p-2 text-sm"
					>
						<button
						...
						<span class="h-5 w-px bg-border"></span>
						<button class="btn btn-ghost" onclick={onResetClick}> Сбросить </button>
					</div>
					<div
						class="mt-3 prose-editor min-h-55"
						use:editorAction={{ onReady, content: state.messageHtml }}
					></div>
				</div>
				<p class="input-help">
					Горячие клавиши: Ctrl/Cmd+B, I, U. Ссылка добавляется через кнопку.
				</p>
...
{#if state.step === 3}
	<section class="grid gap-6 lg:grid-cols-3">
		<div class="card space-y-4 lg:col-span-2">
			<div class="flex flex-wrap items-center justify-between gap-2">
				...
				<div class="flex flex-wrap items-center gap-2">
					{#if state.status?.state === 'stopped'}
						<button class="btn btn-primary px-3 py-1.5 text-sm" onclick={onResumeClick}
							>Продолжить</button
						>
					{/if}
					{#if state.status?.state === 'running' || state.status?.state === 'queued' || state.status?.state === 'stopping'}
						<button class="btn btn-secondary px-3 py-1.5 text-sm" onclick={onPauseClick}
							>Пауза</button
						>
					{/if}
			...
			<div class="grid gap-2">
				...
				<p class="body-m">
					Создано: <span class="text-text-muted">{formatDate(state.status?.createdAt)}</span>
				</p>
				<p class="body-m">
					Старт: <span class="text-text-muted">{formatDate(state.status?.startedAt)}</span>
				</p>
				<p class="body-m">
					Финиш: <span class="text-text-muted">{formatDate(state.status?.finishedAt)}</span>
				</p>
			</div>
		</div>
		...
		<div class="card space-y-4">
			...
			<div class="grid gap-2">
				...
				<p class="body-m">
					Успешно: <span class="text-text-muted">{state.status?.success ?? 0}</span>
				</p>
				<p class="body-m">
					Ошибки: <span class="text-text-muted">{state.status?.failed ?? 0}</span>
				</p>
				<p class="body-m">
					Пропущено: <span class="text-text-muted">{state.status?.skipped ?? 0}</span>
				</p>
			</div>
		</div>
	</section>
	...
	<section class="grid gap-6 lg:grid-cols-2">
		<div class="card space-y-4">
			...
			<div
				class="max-h-72 space-y-2 overflow-y-auto rounded-md border border-border bg-surface-2 p-4 text-sm"
			>
